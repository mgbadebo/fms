...
class FactoryAssetService
{
    ...
    /**
     * Create an Asset record for a factory when not provided.
     *
     * @param int $farmId
     * @param int $siteId
     ...
     * @return Asset
     */
    public function createAssetForFactory(int $farmId, int $siteId, string $factoryName, array $assetData = []): Asset
    {
        // Resolve FACTORY category (global or per farm)
        // Use provided category_id if available (prefix removed by controller), otherwise default to FACTORY
        $categoryId = $assetData['category_id'] ?? null;
        
        if ($categoryId) {
            $category = AssetCategory::find($categoryId);
        } else {
            $category = AssetCategory::where('code', 'FACTORY')->first();
            if (!$category) {
        ...
        $assetFields = [
            ...
            'name' => $assetData['name'] ?? $factoryName,
