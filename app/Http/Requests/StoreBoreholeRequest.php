...
class StoreBoreholeRequest extends FormRequest
{
    public function authorize(): bool
    {
        ...
        // Must have permission and belong to farm of the site
        $siteId = $this->input('site_id');
        if (!$siteId) return false;
        $site = Site::find($siteId);
        if (!$site) return false;
        return $user->can('boreholes.create') && $user->farms()->where('farms.id', $site->farm_id)->exists();
    }
    ...
    public function rules(): array
    {
        ...
        $rules = [
            ...
            'status' => ['required','in:ACTIVE,INACTIVE,UNDER_REPAIR,DECOMMISSIONED'],
